...
async def staff_add(message: types.Message, state: FSMContext):
    ...
    if await sync_to_async(user_request.has_superuser_status)() and await sync_to_async(user_request.has_staff_status)():
        try:
            if current_state is None:
            ...
            await bot.send_message(chat_id=message.from_user.id, text="You need to provide an user id. Use the command like this: /staff <id>")
        except:
